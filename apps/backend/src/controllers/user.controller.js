...
import { editUserHandler, getAllUsersHandler, loginGenerateOtpHandler, loginOtpHandler, passwordResetGenertorHandler, passwordResetHandler, signUpHandler, successGoogleAuthHandler, } from '../handlers/user.handler';
export const signUp = async (req, res, next) => {
    try {
        ...
        if (!process.env.JWT_SECRET)
            throw new HttpError('No JWT Secret', 500);
...
export const loginGenerateOtp = async (req, res, next) => {
    try {
...
export const passwordResetGenertor = async (req, res, next) => {
    try {
        if (!req.body.email)
            throw new HttpError('General Error', 500);
...
export const passwordReset = async (req, res, next) => {
    try {
        ...
        if (!infoReset.newPassword)
            throw new HttpError('New password doesnt exist', 500);
...
export const callbackGoogleAuth = async (req, res, next) => {
...
export const failedgoogleauth = (_req, res) => {
    res.send('Failed');
};
export const successgoogleauth = (req, res) => {
    if (req.user) {
        const userInfo = {
            _id: req.user._id,
            email: req.user.email,
